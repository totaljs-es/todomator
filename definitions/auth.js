...
ON('start', function() {
	...
	opt.onread = async function(meta, next) {
		...
		if (session) {
			var user = await DATA.read('tbl_user').fields('id,language,name,photo,email,notifications,permissions,sa,is2fa').id(meta.userid).where('isdisabled=FALSE AND isinactive=FALSE AND isremoved=FALSE').promise();
			if (user) {
